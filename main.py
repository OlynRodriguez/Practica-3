...
class protocoloFTP(object):
    def __init__(self,p='null',u='null') -> object:
        self.pw=p
        self.user=u
    ...
    def sSET(self, conn):
        ...
        fileName = "archivos" + str(img)
        fileObj = Path(fileName)
        if fileObj.is_file():
            logging.debug('Respuesta: 213, el archivo existe')
            conn.send(str('213').encode('utf-8'))
            self.mandar(conn,img)

        else:
            logging.debug('Respuesta: 501,no existe archivo')
